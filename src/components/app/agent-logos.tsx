/* ---------------------------------------------------------------------------
   ...
   Real logos, not approximations. Every path below is the official mark, taken
   verbatim from @lobehub/icons-static-svg and simple-icons (both of which
   publish the vendors' own geometry) and inlined here so there is no runtime
   dependency and no request to somebody else's CDN on a dashboard page.

   Nominative use: these identify the tools Voicebox connects to. Each is the
   property of its owner, is rendered unmodified, and is not used as our own
   branding.

   Colour: the app ships dark-only (see the `dark` class on <html>), so each
   mark carries the treatment its owner uses on a dark ground. Claude keeps its
   own orange; the marks whose brand colour is black are drawn in white, which
   is the vendors' own dark-background version, not an invention of ours.
--------------------------------------------------------------------------- */

type LogoProps = { className?: string };

/** All marks share a 24-grid, so they optically match at any one size. */
function Mark({
  d,
  className,
  title,
...
export const ClaudeCodeLogo = ({ className }: LogoProps) => (
  <Mark d={CLAUDE_CODE_D} className={className} title="Claude Code" />
);
export const ClaudeLogo = ({ className }: LogoProps) => (
  <Mark d={CLAUDE_D} className={className} title="Claude" />
);
export const CodexLogo = ({ className }: LogoProps) => (
  <Mark d={CODEX_D} className={className} title="Codex" />
);
export const CursorLogo = ({ className }: LogoProps) => (
  <Mark d={CURSOR_D} className={className} title="Cursor" />
);
export const WindsurfLogo = ({ className }: LogoProps) => (
  <Mark d={WINDSURF_D} className={className} title="Windsurf" />
);
export const CopilotLogo = ({ className }: LogoProps) => (
  <Mark d={COPILOT_D} className={className} title="GitHub Copilot" />
);
